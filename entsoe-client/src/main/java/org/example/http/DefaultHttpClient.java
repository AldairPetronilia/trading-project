...
public class DefaultHttpClient implements HttpClient {
  ...
  public DefaultHttpClient(EntsoEClientConfiguration config) {
    this.config = config;
    this.httpClient =
        java.net.http.HttpClient.newBuilder().connectTimeout(config.getConnectionTimeout()).build();
  ...
  public String get(String url, Map<String, String> parameters) throws HttpClientException {
    parameters.put("securityToken", config.getApiToken());
        ...
        () -> {
          ...
          try {
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
              ...
              String errorMsg =
                  String.format("Request failed with status %d", response.statusCode());
  ...
  private HttpRequest buildRequest(String url) throws HttpClientException {
    try {
      HttpRequest.Builder builder =
          HttpRequest.newBuilder().uri(new URI(url)).timeout(config.getReadTimeout()).GET();
